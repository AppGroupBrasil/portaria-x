...
import {
  ...
  ArrowRight, Phone, Mail, MessageCircle, Zap, Lock, Eye, Fingerprint,
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif", color: t.text1, background: t.bg1, transition: "background 0.4s, color 0.4s" }}>
        ...
        <div className="landing-section">
          <div style={{ textAlign: "center", marginBottom: "60px" }}>
            ...
            <p style={{ color: t.text3, fontSize: "16px", maxWidth: "560px", margin: "0 auto", lineHeight: 1.7 }}>
              Cada funcionalidade atende portaria, morador e síndico. Veja quem usa cada recurso.
            </p>
            {/* Profile legend */}
            <div style={{ display: "flex", gap: "12px", justifyContent: "center", marginTop: "24px", flexWrap: "wrap" }}>
              {(["portaria", "morador", "sindico"] as ProfileKey[]).map((k) => {
                ...
                return (
                  <div key={k} onClick={() => navigate(pr.link)} style={{ display: "flex", alignItems: "center", gap: "6px", background: pr.bg, padding: "6px 14px", borderRadius: "999px", border: "none", boxShadow: `0 2px 8px ${pr.color}40`, cursor: "pointer", transition: "transform 0.15s" }}
            ...
            {allFeatures.map((f) => {
              ...
              return (
                <div
                  key={f.title}
                  onClick={() => {
                    if (f.title === "Portaria Virtual (IoT)") { navigate("/portaria-virtual-tutorial"); }
                ...
                >
                  {f.badge && (
                    <span style={{
                      ...
                      background: "transparent", border: mode === "dark" ? "1.5px solid #ffffff" : "1.5px solid #003580",
                      color: mode === "dark" ? "#ffffff" : "#003580", fontSize: "11px", fontWeight: 700,
                  ...
                  <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "16px", paddingTop: "14px", borderTop: `1px solid ${mode === "dark" ? "rgba(255,255,255,0.08)" : "rgba(0,53,128,0.08)"}` }}>
                    {f.profiles.map((pk) => {
                      ...
                      const PrIcon = pr.icon;
                      return (
                        <div key={pk} onClick={(e) => { e.stopPropagation(); navigate(pr.link); }} style={{
      ...
      <section style={{ background: mode === "dark" ? "#ffffff" : "#003580", padding: "80px 24px", transition: "background 0.4s" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
          {/* Badge */}
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: mode === "dark" ? "#003580" : "rgba(255,255,255,0.1)", border: mode === "dark" ? "1.5px solid #ffffff" : "1.5px solid #ffffff", borderRadius: "999px", padding: "8px 20px", marginBottom: "24px", fontSize: "14px", fontWeight: 600, color: mode === "dark" ? "#ffffff" : "#ffffff" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "20px" }}>
            ...
            <div style={{
              ...
              onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.borderColor = mode === "dark" ? "rgba(255,255,255,0.6)" : "rgba(0,53,128,0.6)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.borderColor = mode === "dark" ? "rgba(255,255,255,0.3)" : "rgba(0,53,128,0.3)"; }}
              onClick={() => { navigator.clipboard.writeText(window.location.origin + "/apresentacao"); }}
            >
      ...
      <section id="precos" style={{ background: t.bg1, transition: "background 0.4s" }}>
        <div className="landing-section">
          ...
          <div style={{ marginTop: "40px", maxWidth: "700px", margin: "40px auto 0", display: "flex", flexDirection: "column", gap: "16px" }}>
            {[
              { icon: DoorOpen, title: "Portaria Virtual (IoT)", desc: "Abra portões e portas pelo app com ESP32. Adicione a qualquer plano por apenas:", price: "R$200", link: "/portaria-virtual-tutorial" },
              { icon: Eye, title: "Leitura de Placa Veicular por Câmera IP", desc: "Câmera IP lê a placa automaticamente na entrada e saída. Identifica veículos e libera acesso. <strong style='color:#ffffff'>Pelo celular já incluso no plano.</strong>", price: "R$200", link: "/portaria-virtual-tutorial" },
              { icon: Fingerprint, title: "Biometria Facial por Câmera IP", desc: "Reconhecimento facial via câmera IP na entrada. Identificação automática sem contato. <strong style='color:#ffffff'>Pelo celular já incluso no plano.</strong>", price: "R$200", link: "/portaria-virtual-tutorial" },
            ].map((addon, idx) => (
              <div key={idx} onClick={() => navigate(addon.link)} style={{
                background: mode === "dark" ? "transparent" : "#003580",
                border: `3px solid ${mode === "dark" ? "rgba(255,255,255,0.6)" : "#ffffff"}`, borderRadius: "16px",
                padding: "24px 28px", display: "flex", alignItems: "center", gap: "16px",
                flexWrap: "wrap", cursor: "pointer",
              }}>
                <div style={{
                  width: "48px", height: "48px", borderRadius: "12px",
                  background: mode === "dark" ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.15)",
                  display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                }}>
                  <addon.icon style={{ width: "24px", height: "24px", color: mode === "dark" ? "#f1f5f9" : "#ffffff" }} />
                </div>
                <div style={{ flex: 1, minWidth: "200px" }}>
                  <p style={{ fontWeight: 700, fontSize: "16px", color: mode === "dark" ? "#f1f5f9" : "#ffffff", marginBottom: "4px" }}>
                    {addon.title}
                  </p>
                ...
                <div style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
                  <div>
                    <span style={{ fontSize: "32px", fontWeight: 900, color: mode === "dark" ? "#f1f5f9" : "#ffffff" }}>+{addon.price}</span>
                    <span style={{ fontSize: "13px", color: mode === "dark" ? t.text3 : "rgba(255,255,255,0.8)", display: "block" }}>/mês</span>
                  </div>
                  <button
