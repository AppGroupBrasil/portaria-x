...
router.put("/tokens/:id/regenerate", authenticate, authorize("master", "administradora", "sindico"), (req: Request, res: Response) => {
  try {
    ...
    ).get(Number.parseInt(req.params.id as string), req.user!.condominio_id) as any;
...
router.delete("/tokens/:id", authenticate, authorize("master", "administradora", "sindico"), (req: Request, res: Response) => {
  try {
    ...
    ).run(Number.parseInt(req.params.id as string), req.user!.condominio_id);
...
function groupMoradoresByUnit(moradores: any[]) {
  ...
  for (const m of moradores) {
    ...
    const apt = apartments.get(unit);
    if (apt) apt.moradores.push(moradorEntry);
  }
...
router.get("/public/:token", (req: Request, res: Response) => {
  try {
    ...
    if (tokenRow.tipo === "condominio") {
      ...
      const blocos = allBlocks.map((block: any) => ({
        id: block.id,
        nome: block.name,
        apartamentos: groupMoradoresByUnit(allMoradores.filter((m: any) => m.block === block.name)),
      }));
    ...
    res.json({
      ...
      apartamentos: groupMoradoresByUnit(moradores),
    });
  } catch (err: any) {
    console.error("Erro em interfone:", err);
    res.status(500).json({ error: "Erro interno do servidor" });
  ...
  try {
    const funcionarios = db.prepare(
      `SELECT id, name FROM users
       ...
       ORDER BY name`
    ).all(Number.parseInt(req.params.condominioId as string)) as any[];
...
router.get("/public/security/:moradorId", (req: Request, res: Response) => {
  try {
    ...
    ).get(Number.parseInt(req.params.moradorId as string)) as any;
  ...
  try {
    ...
    if (existing) {
      db.prepare(
        ...
        horario_silencioso_fim || null,
        bloqueados || "[]",
        whatsapp_interfone === undefined ? null : whatsapp_interfone,
        req.user!.id
      );
    } else {
      db.prepare(
...
router.put("/calls/:id", (req: Request, res: Response) => {
  try {
    ...
    const numId = Number.parseInt(idParam);
    if (!Number.isNaN(numId) && String(numId) === idParam) {
      params.push(numId);
    } else {
...
// ═══════════════════════════════════════════════════════════
// MORADORES — List moradores for internal call
// ═══════════════════════════════════════════════════════════
